...
#missmap(prepro_withoutNA, rank.order = FALSE, col = c(0,1), legend = FALSE) # bug with visualization package
```
...
Por problemas de configuración en las depencias del paquete keras y tensor instaladas en la máquina local  hemos decidido realizar estos pasos en google [collab]collab(https://colab.research.google.com/drive/1kD7DqRHyqfN45XrK2V03_POWMkzn6azS#scrollTo=CTAHnzFcmwfx)
...
Los resultados distan de lo esperado ya que el dataset que podría parecer menos completo a priori consigue los mejores resultados. Aunque estas diferencias no son signifacativas es curiosos como el tratamiento de missing values debe hacerse de manera conciencuda. En este caso creemos que las muestras con missing values eran erróneas desde un principio, por tanto lo mejor ha sido eliminarlas. Al imputar valores con knn o pmm hemos sufrido un coste computacional muy grande para simplemente completar muestras que desde un princpio no eran adecuadas para el entrenamiento de nuestro modelo pues se alejaban de la realidad.
...
![Imagen SVM Kernels](./results.png)
